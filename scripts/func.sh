#!/usr/bin/env bash
#
# Copyright (C) 2022 Ing <https://github.com/syno-community>
#
...
function getBuildroot() {
  ...
  if [ "${1}" = "latest" ]; then
    TAG=$(curl -s "https://api.github.com/repos/syno-community/rr-lkms/releases" | jq -r ".[0].tag_name")
  fi
  [ ! -d "${DEST_PATH}" ] && mkdir -p "${DEST_PATH}"
  rm -rf "${DEST_PATH}/bzImage-rr"
  STATUS=$(curl -w "%{http_code}" -L "https://github.com/syno-community/rr-buildroot/releases/download/${TAG}/bzImage" -o "${DEST_PATH}/bzImage-rr")
  echo "TAG=${TAG}; Status=${STATUS}"
  [ ${STATUS} -ne 200 ] && exit 1

  rm -rf "${DEST_PATH}/initrd-rr"
  STATUS=$(curl -w "%{http_code}" -L "https://github.com/syno-community/rr-buildroot/releases/download/${TAG}/rootfs.cpio.xz" -o "${DEST_PATH}/initrd-rr")
  echo "TAG=${TAG}; Status=${STATUS}"
  [ ${STATUS} -ne 200 ] && exit 1
...
function getLKMs() {
  ...
  if [ "${2}" = "true" ]; then
    ...
    TAG=$(curl -s "https://api.github.com/repos/syno-community/rr-lkms/releases/latest" | jq -r ".tag_name")
  fi
  STATUS=$(curl -w "%{http_code}" -L "https://github.com/syno-community/rr-lkms/releases/download/${TAG}/rp-lkms.zip" -o "${CACHE_FILE}")
  echo "TAG=${TAG}; Status=${STATUS}"
  [ ${STATUS} -ne 200 ] && exit 1
  # Unzip LKMs
  rm -rf "${DEST_PATH}"
...
function getAddons() {
  ...
  if [ "${2}" = "true" ]; then
    TAG=$(curl -s "https://api.github.com/repos/syno-community/rr-addons/releases" | jq -r ".[0].tag_name")
  else
    TAG=$(curl -s "https://api.github.com/repos/syno-community/rr-addons/releases/latest" | jq -r ".tag_name")
  fi
  STATUS=$(curl -w "%{http_code}" -L "https://github.com/syno-community/rr-addons/releases/download/${TAG}/addons.zip" -o "${CACHE_FILE}")
  echo "TAG=${TAG}; Status=${STATUS}"
  [ ${STATUS} -ne 200 ] && exit 1
  rm -rf "${DEST_PATH}"
  mkdir -p "${DEST_PATH}"
...
function getModules() {
  ...
  rm -f "${CACHE_FILE}"
  if [ "${2}" = "true" ]; then
    TAG=$(curl -s "https://api.github.com/repos/syno-community/rr-modules/releases" | jq -r ".[0].tag_name")
  else
    TAG=$(curl -s "https://api.github.com/repos/syno-community/rr-modules/releases/latest" | jq -r ".tag_name")
  fi
  STATUS=$(curl -w "%{http_code}" -L "https://github.com/syno-community/rr-modules/releases/download/${TAG}/modules.zip" -o "${CACHE_FILE}")
  echo "TAG=${TAG}; Status=${STATUS}"
  [ ${STATUS} -ne 200 ] && exit 1
  # Unzip Modules
  rm -rf "${DEST_PATH}"
